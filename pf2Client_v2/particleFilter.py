...
class filtroParticulas():
    def __init__(self, map, n_part=4000):
        self.n_part = n_part
        ...
        for i in range (self.n_part):
            # Orientacao random
            # self.particulas.append( particula( random.random() * (self.mapmax_x), random.random() * (self.mapmax_y), random.random()*360, 1))

            # Orientacao 0
            self.particulas.append( particula( random.random() * (self.mapmax_x), random.random() * (self.mapmax_y), 0, 1))
    ...
    def weights_calculation(self, LINEsens, DISTsens, metodo):
        ...
        if metodo == 1:
            for i,particula in enumerate(self.particulas):  
                sensorDIST_center_index =( int(self.x_offset+self.map_scale_factor*particula.sensorDIST_center_posx), int(self.y_offset+self.map_scale_factor*particula.sensorDIST_center_posy) )
                sensorDIST_left_index = ( int(self.x_offset+self.map_scale_factor*particula.sensorDIST_left_posx), int(self.y_offset+self.map_scale_factor*particula.sensorDIST_left_posy) )
                sensorDIST_right_index = ( int(self.x_offset+self.map_scale_factor*particula.sensorDIST_right_posx), int(self.y_offset+self.map_scale_factor*particula.sensorDIST_right_posy) )
                sensorDIST_back_index = ( int(self.x_offset+self.map_scale_factor*particula.sensorDIST_back_posx), int(self.y_offset+self.map_scale_factor*particula.sensorDIST_back_posy) )
